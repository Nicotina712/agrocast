"""
US30 Dow Jones Robot — Microstructure Features
Adapts the feature pipeline for US30 (Dow Jones Industrial Average) CFD.

US30 differences vs crypto:
  - Trades only Mon-Fri NYSE hours (09:30-16:00 ET = 08:30-15:00 CT)
  - High absolute price (~38,000-45,000 pts), ATR 5m: typically 30-100 pts
  - Key round numbers: every 1,000 pts and 5,000 pts
  - MT5 returns tick_volume for indices (not volume)
  - No weekend data
"""
...
def build_intraday_features(df_bars: pd.DataFrame, interval: str = "5m") -> pd.DataFrame:
    ...
    # Volume (tick_volume for indices)
    ...
    # US30-specific: distance to round numbers (1,000 and 5,000 pt steps)
    round_1000 = (close / 1000).round() * 1000
    feats["dist_round1k"]  = (close - round_1000) / 1000
    round_5000 = (close / 5000).round() * 5000
    feats["dist_round5k"]  = (close - round_5000) / 5000
...
def summarize_bars(df_bars: pd.DataFrame, df_feats: pd.DataFrame) -> dict:
    ...
    for i in range(min(6, len(df_bars))):
        ...
        recent_bars.append({
            "i":     -i,
            "open":  round(float(b["open"]),  1),
            "high":  round(float(b["high"]),  1),
            "low":   round(float(b["low"]),   1),
            "close": round(float(b["close"]), 1),
            "vol":   vol_val,
    ...
    return {
        ...
        "session_range": {
            "high":  round(session_high, 1),
            "low":   round(session_low,  1),
            "range": round(session_high - session_low, 1),
        },
        "indicators": {
            "rsi14":          round(float(latest.get("rsi14", 50)), 1),
            "atr14_pts":      round(float(latest.get("atr14", 0)),  1),
            ...
            "ema9":           round(float(latest.get("ema9", current_price)), 1),
            "ema21":          round(float(latest.get("ema21", current_price)), 1),
            "ema50":          round(float(latest.get("ema50", current_price)), 1),
            ...
            "dist_round1k":   round(float(latest.get("dist_round1k", 0)), 3),
            "dist_round5k":   round(float(latest.get("dist_round5k", 0)), 3),
        },
        "returns": {
