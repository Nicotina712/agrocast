"""
US30 Dow Jones â€” Live Runner
Mon-Fri, prime session 08:30-15:00 CT (09:30-16:00 ET).
...
from config import (
    ...
    MAX_ENTRY_SLIP_PCT,
    DEFAULT_VOLUME,
    SL_ATR_MULT, TP_ATR_MULT, VWAP_TH, calc_lots,
...
def _log_paper(sig, summary):
    ...
    entry = {
        ...
        "us30_regime":sig.get("us30_regime"),"risk_on_off":sig.get("risk_on_off"),
...
def _build_fundamental_context():
    ct = _now_ct()
    return {
        "vix_level":         "unknown â€” check CBOE VIX",
        "fed_policy":        "unknown â€” FOMC stance",
        "industrial_outlook":"unknown â€” ISM Manufacturing, industrial stocks",
        "financial_sector":  "unknown â€” JPMorgan, Goldman, BofA",
        "us_macro":          "unknown â€” NFP, GDP, retail sales",
        "dxy_trend":         "unknown â€” DXY",
        "session_note":      f"US30 market hours | CT: {ct.strftime('%H:%M')} | weekday: {ct.weekday()}",
        "key_levels_note":   "Watch: 38k, 40k, 42k, 44k, 45k round levels",
    }
...
def run_cycle(execute=False):
    ...
    print(f"\n{'='*60}")
    print(f"US30 Dow Jones â€” Cycle at {ct.strftime('%Y-%m-%d %H:%M')} CT")
    print(f"Prime: {is_prime_now} | Weekend: {is_wknd} | Mins left: {mins_left}")
    print("="*60)

    if not _is_trading_day():
        print("Weekend â€” US30 market closed. Skip.")
    ...
    # RESILIENCIA tope-diario (2026-06-16): mecanico-primario -> si se agoto la cuota LLM, NO ir dark;
    # seguir con la mecanica VWAP-rev sin veto. Solo se marca _llm_capped.
    _llm_capped = not can_call
    if _llm_capped:
        print(f"LLM gate: {reason} -> sigo con mecanica VWAP-rev sin veto (resiliencia tope)")
        _log("llm_gate", {"reason": reason, "state_date": state.get("date"),
                          "llm_calls": state.get("llm_calls"), "fallback": "mecanico sin veto"})
    ...
    # ── MECHANICAL-PRIMARY (VWAP mean-reversion 1h) + LLM-VETO (2026-06-16) ──────
    # US30 (Dow) migrado de LLM-puro a mecanico-primario: el Dow es MEAN-REVERTING en sesion cash
    # (busqueda+validacion 06-16: VWAP-rev 1h th1.0 SL1.5/TP3.0, expR_net +0.35, meanPF 2.21,
    # consist 75%, minPF 0.82, placebo edge +0.37). La señal de reversion MANDA; el LLM solo VETA
    # si ve tendencia opuesta FUERTE (HIGH). Gana resiliencia ante cortes de API (ya no queda dark).
    from portfolio_guard import mechanical_signal as _mech
    _m = _mech(bars, mode="vwap_reversion", sl_atr=SL_ATR_MULT, tp_atr=TP_ATR_MULT, vwap_th=VWAP_TH)

    # LLM como VETO best-effort: si esta capado o cae, NO bloquea (resiliencia).
    _llm_dir, _llm_conf = "FLAT", "LOW"
    if _m:
        try:
            if _llm_capped:
                raise RuntimeError("cuota LLM agotada (tope diario)")
            print("Calling Trend Agent (US30, veto)...")
    ...
    if not _m:
        signal = {"signal": "FLAT", "confidence": "LOW", "reasoning": "sin extension VWAP (no fade)"}
    elif _llm_dir in ("LONG", "SHORT") and _llm_dir != _m["signal"] and _llm_conf == "HIGH":
        # mean-reversion es contra-tendencia; solo vetar si el LLM ve tendencia opuesta FUERTE.
        ...
        _stop_pts = abs(_m["entry"] - _m["sl"]); _lots = calc_lots(_stop_pts)
        signal = {
    ...
    if signal["signal"] != "FLAT":
        ...
        if _slp < 0.35:
            ...
            print(f"[SL GUARD] SL too tight ({_slp:.3f}% < 0.35%). "
    ...
    # -- F. Counter-trend quality filter (US30) --------------------------------
    if signal["signal"] != "FLAT":
        _sig_f  = signal["signal"]
        _conf_f = signal.get("confidence", "LOW")
        _reg_f  = signal.get("us30_regime", "")
        ...
        if _desc_f:
            ...
            if not _allow_f:
                ...
                _log("trend_filter_veto", {"signal": _sig_f, "us30_regime": _reg_f, "confidence": _conf_f, "rr": _rr_f})
                signal["signal"] = "FLAT"
    ...
    # -------------------------------------------------------------------------
    # llm_calls ya se conto arriba (solo cuando hubo llamada real de veto). Aqui solo ciclo.
    ...
    _save_state(state)
    _save_signal(signal)
    _log("signal",{k:signal.get(k) for k in ("signal","entry","sl","tp","lots","confidence","rr","risk_usd","us30_regime")})
    ...
    if signal["signal"] != "FLAT":
        ...
        print(f"    Regime: {signal.get('us30_regime')} | Risk: {signal.get('risk_on_off')} | Conf: {signal.get('confidence')}")
    else:
    ...
    if execute and signal["signal"] != "FLAT":
        ...
        result = place_order(
            ...
            sl=_sl, tp=_tp,
            comment="US30_QA", magic=OUR_MAGIC, dry_run=False,
        )
...
def main():
    ...
    if execute:
        print("LIVE EXECUTION ENABLED (US30 demo)")

    if args.loop:
        print(f"US30 loop. Cycle: {CYCLE_MINUTES} min. Prime: {_PRIME_START[0]:02d}:{_PRIME_START[1]:02d}-{_PRIME_END[0]:02d}:{_PRIME_END[1]:02d} CT (Mon-Fri)")
        os.makedirs(ARTIFACTS_DIR, exist_ok=True)
        ...
        while True:
            try:
                if _is_trading_day():
                    ...
                    print("Weekend â€” US30 closed. Skip.")
