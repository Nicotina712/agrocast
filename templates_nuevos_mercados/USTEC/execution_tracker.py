...
def compute_performance(trades):
    ...
    regimes = {}
    for t in trades:
        r = t.get("ustec_regime","unknown"); regimes[r] = regimes.get(r,0)+1
    extra_dist = {}
    for t in trades:
        e = t.get("ai_sentiment","unknown"); extra_dist[e] = extra_dist.get(e,0)+1
    return {
...
def print_report():
    ...
    print("\n" + "="*58)
    print(f"  USTEC Robot — Performance Report")
    print("="*58)
    ...
    print(f"  AI Sent dist    : {stats.get('extra_dist',{})}")
...
def main():
    parser = argparse.ArgumentParser(description="USTEC Execution Tracker")
    parser.add_argument("--report", action="store_true")
    parser.add_argument("--sync",   action="store_true")
    args = parser.parse_args()
    if args.sync:
        fills = sync_fills()
        print(f"Open USTEC positions (magic={OUR_MAGIC}): {len(fills)}")
