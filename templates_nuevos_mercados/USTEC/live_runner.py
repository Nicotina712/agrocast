"""
USTEC Nasdaq 100 â€” Live Runner
...
def _experiment_active(name):
    """Lee data/experiments.json (kill-switch). True si el experimento sigue activo."""
    try:
        import json as _j
...
def _log_paper(sig, summary):
    ...
    entry = {
        ...
        "ustec_regime":sig.get("ustec_regime"),"ai_sentiment":sig.get("ai_sentiment"),
...
def _build_fundamental_context():
    ct = _now_ct()
    return {
        "nvda_weight":    "unknown â€” NVDA ~10% of USTEC, check earnings",
        "ai_sentiment":   "unknown â€” OpenAI/Anthropic/Google AI news",
        "fed_rates":      "unknown â€” inverse correlation with tech/growth",
        "mag7_momentum":  "unknown â€” check AAPL/MSFT/NVDA/GOOGL/META/AMZN/TSLA",
        "tech_regulation":"unknown â€” antitrust, privacy, AI regulation",
        "vix_level":      "unknown â€” VIX > 20 = headwind for longs",
        "session_note":   f"USTEC market hours | CT: {ct.strftime('%H:%M')} | weekday: {ct.weekday()}",
        "key_levels_note":"Watch: 17k, 18k, 19k, 20k, 21k, 22k round levels",
    }
...
def run_cycle(execute=False):
    ...
    print(f"\n{'='*60}")
    print(f"USTEC Nasdaq 100 â€” Cycle at {ct.strftime('%Y-%m-%d %H:%M')} CT")
    print(f"Prime: {is_prime_now} | Weekend: {is_wknd} | Mins left: {mins_left}")
    print("="*60)

    if not _is_trading_day():
        print("Weekend â€” USTEC market closed. Skip.")
    ...
    try:
        ...
        _vf = _avf(bars, zscore_threshold=VOL_ZSCORE_THRESHOLD)
        if _vf:
    ...
    # ── MECHANICAL-PRIMARY (trend EMA20/50 30m) + LLM-VETO GATEADO (migrado 2026-06-26) ──
    # USTEC era LLM-puro y NUNCA operaba (0 trades; LLM ultra-selectivo + sin fallback = dark al capar).
    # Validado WF 8 folds (meanPF 2.18, expR +0.82, consist 75%, vecindad 20/50-20/100 robusta) +
    # placebo p99.4 (señal real +0.30 vs random −0.02). La señal mecánica MANDA; el LLM solo VETA si ve
    # tendencia opuesta FUERTE (HIGH). GATEADO: solo gasta LLM si hay señal mecánica que vetar.
    from portfolio_guard import mechanical_signal as _mech
    from config import EMA_FAST as _EF, EMA_SLOW as _ES, SL_ATR_MULT as _SLM, TP_ATR_MULT as _TPM, calc_lots as _calc_lots
    _m = _mech(bars, mode="trend", sl_atr=_SLM, tp_atr=_TPM, ema_fast=_EF, ema_slow=_ES)
    _llm_dir, _llm_conf = "FLAT", "LOW"
    trend, risk = {}, {}
    if _m and not _llm_capped:
        try:
            print("Calling Trend+Risk Agent (USTEC, veto)...")
            ...
            state["llm_calls"] += 2
            print(f"LLM veto-check: {_llm_dir} ({_llm_conf})")
        except Exception as _exc:
            _log("agent_error", {"error": str(_exc)[:120], "fallback": "mecanico sin veto"})
            print(f"[RESILIENCIA] LLM caido/capado ({str(_exc)[:50]}) -> mecanica sin veto")

    if not _m:
        signal = {"signal": "FLAT", "confidence": "LOW", "reasoning": "sin señal mecanica trend"}
    elif _llm_dir in ("LONG", "SHORT") and _llm_dir != _m["signal"] and _llm_conf == "HIGH":
        print(f"[LLM VETO] LLM ve {_llm_dir} HIGH contra mech {_m['signal']} -> FLAT")
        ...
        signal = {
            ...
            "confidence": "MEDIUM", "reasoning": f"mech trend {_m['basis']} | LLM={_llm_dir}",
        }
    signal.update({"timestamp":datetime.now().isoformat(),"symbol":SYMBOL,"price":price})
    # ── FILTRO DE CONTEXTO EN SOMBRA (USTEC←US2000, Δ+0.15 en mecánica; señal viva=LLM no replicable
    # → solo LOGUEAMOS qué filtraría, NO actuamos. Activar tras validar en forward test). 2026-06-14
    if signal["signal"] != "FLAT":
        try:
            from portfolio_guard import context_aligned as _ctxa
            _log("ctx_shadow", {"signal": signal["signal"], "proxy": "US2000",
                                "aligned": _ctxa("US2000", signal["signal"])})
        except Exception:
            pass
    ...
    # -- F. Counter-trend quality filter (USTEC) --------------------------------
    if signal["signal"] != "FLAT":
        _sig_f  = signal["signal"]
        _conf_f = signal.get("confidence", "LOW")
        _reg_f  = signal.get("ustec_regime", "")
        _rr_raw_f = str(signal.get("rr") or "0").split(":")[0].split("/")[0].strip()
        _rr_f   = float(_rr_raw_f) if _rr_raw_f else 0
        _counter_f = {
        ...
        _desc_f = _counter_f.get((_sig_f, _reg_f))
        if _desc_f:
            _allow_f = False
            if _conf_f == "HIGH":
                _allow_f = True
            elif _conf_f == "MEDIUM" and _reg_f == "DISTRIBUTION" and _rr_f >= 2.5:
                _allow_f = True
            elif _conf_f == "MEDIUM" and _experiment_active("ustec_take_medium"):
                # EXPERIMENTO 2026-06-17 (kill-switch): permitir MEDIUM counter-trend en demo.
                _allow_f = True
                print("[EXPERIMENT] ustec_take_medium activo -> permito MEDIUM counter-trend")
            if not _allow_f:
                print(f"[TREND FILTER] {_sig_f} contra {_reg_f} conf={_conf_f} -- veto")
                _log("trend_filter_veto", {"signal": _sig_f, "ustec_regime": _reg_f, "confidence": _conf_f, "rr": _rr_f})
                signal["signal"] = "FLAT"
                signal["veto_reason"] = f"Counter-trend {_sig_f} en {_reg_f} requiere HIGH confidence"
            else:
                print(f"[TREND FILTER] {_sig_f} contra {_reg_f} PERMITIDO (conf={_conf_f})")
    ...
    # -------------------------------------------------------------------------
    state["cycles"] += 1; state["last_signal_time"] = signal["timestamp"]  # llm_calls se incrementa en el bloque de veto (solo si se llamó)
    ...
    _save_state(state)
    _save_signal(signal)
    _log("signal",{k:signal.get(k) for k in ("signal","entry","sl","tp","lots","confidence","rr","risk_usd","ustec_regime")})
    ...
    if signal["signal"] != "FLAT":
        ...
        print(f"    Regime: {signal.get('ustec_regime')} | AI: {signal.get('ai_sentiment')} | Conf: {signal.get('confidence')}")
    else:
    ...
    if execute and signal["signal"] != "FLAT":
        ...
        result = place_order(
            ...
            sl=_sl, tp=_tp,
            comment="USTEC_QA", magic=OUR_MAGIC, dry_run=False,
        )
...
def main():
    ...
    if execute:
        print("LIVE EXECUTION ENABLED (USTEC demo)")

    if args.loop:
        print(f"USTEC loop. Cycle: {CYCLE_MINUTES} min. Prime: {_PRIME_START[0]:02d}:{_PRIME_START[1]:02d}-{_PRIME_END[0]:02d}:{_PRIME_END[1]:02d} CT (Mon-Fri)")
        os.makedirs(ARTIFACTS_DIR, exist_ok=True)
        ...
        while True:
            try:
                if _is_trading_day():
                    ...
                    print("Weekend â€” USTEC closed. Skip.")
