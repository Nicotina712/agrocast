...
def _build_html():
    ...
    for t in reversed(trades[-20:]):
        ...
        reg = t.get("ustec_regime","?"); ext = t.get("ai_sentiment","?")
        rows += f"""<tr>
          <td>{ts}</td>
...
<title>{EMOJI} USTEC Dashboard</title>
<style>
...
<div class="sig-card">
  ...
  | Régimen: {sig.get("ustec_regime","—")} | Contexto: {sig.get("ai_sentiment","—")}</small><br>
...
def start_dashboard(port=PORT):
    ...
    print(f"USTEC dashboard: http://localhost:{port}")
    srv.serve_forever()
