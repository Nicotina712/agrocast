"""CHINA50 — Web Dashboard (port 8091)"""
...
def _build_html():
    ...
    for t in reversed(trades[-20:]):
        ...
        reg = t.get("sp500_regime","?"); ext = t.get("risk_on_off","?")
        rows += f"""<tr>
          <td>{ts}</td>
...
<title>{EMOJI} CHINA50 Dashboard</title>
<style>
...
<div class="sig-card">
  ...
  | Régimen: {sig.get("sp500_regime","—")} | Contexto: {sig.get("risk_on_off","—")}</small><br>
...
def start_dashboard(port=PORT):
    ...
    print(f"CHINA50 dashboard: http://localhost:{port}")
    srv.serve_forever()
