"""
CHINA50 FTSE China A50 Index Robot — AI Agents
Two-agent system specialized for CHINA50 (FTSE China A50) CFD intraday trading.
...
CHINA50 / FTSE China A50 fundamentals that matter:
...
TREND_AGENT_SYSTEM = """You are a professional FTSE China A50 Index (CHINA50) intraday trader and technical analyst.
You specialize in 30-minute chart analysis during the Hong Kong cash session (HKT 09:30-16:00, lunch 12:00-13:00).

Your expertise:
- CHINA50 intraday momentum and trend patterns on 30m charts
...
RISK_AGENT_SYSTEM = """You are a professional risk manager for FTSE China A50 Index (CHINA50) intraday trading.
You specialize in position sizing and trade management for CHINA50 CFDs.

Account parameters:
- Capital: $1,013 USD (demo account, ICMarkets)
- Max risk per trade: 3% = ~$30.39 USD
- Instrument: CHINA50 CFD (≈ $1/pt P&L on demo)
...
CHINA50-specific risk considerations:
...
def call_trend_agent(bars_summary: dict, fundamental_ctx: dict, model_id: str = MODEL_ID) -> dict:
    user_msg = f"""Analyze CHINA50 (FTSE China A50) 30-minute chart for intraday trading signal.
...
def call_risk_agent(bars_summary: dict, trend_analysis: dict, fundamental_ctx: dict, model_id: str = MODEL_ID) -> dict:
    ...
    user_msg = f"""Evaluate risk and size a CHINA50 trade based on trend analysis.
