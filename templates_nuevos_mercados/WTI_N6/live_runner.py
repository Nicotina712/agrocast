"""
WTI_N6 Crude Oil â€” Live Runner
...
def _build_fundamental_context():
    ct = _now_ct()
    base = {
        "session_note":   f"WTI NYMEX | CT: {ct.strftime('%H:%M')} | weekday: {ct.weekday()}",
        "key_levels_note":"Watch: $65, $70, $75, $80, $85, $90 per barrel",
    }
    # ── 2026-06-24 HILO 1+4: poblar contexto REAL (antes era todo "unknown") ──
    # news-intel agregado (data/news_portfolio) + calendario macro (data/macro_events).
    # Fail-open: si falla la lectura, el LLM recibe el base + 'unavailable' (no rompe el veto).
    try:
        ...
        from news_context_aggregator import news_context as _nc
        from event_context import event_context as _ec
        _n = _nc("WTI_N6")
        base["news_intel"] = {
            "net_direction": _n["label"], "net_score": _n["net"],
            "articles": _n["n"], "bull_bear": f"{_n.get('bull',0)}/{_n.get('bear',0)}",
            "top_drivers": _n["drivers"], "age_hours": _n["age_h"],
        }
        base["macro_calendar"] = _ec("WTI_Q6", within_hours=12)
    except Exception as _e:
        base["news_intel"] = "unavailable"
        base["macro_calendar"] = "unavailable"
    return base
...
def run_cycle(execute=False):
    ...
    print(f"\n{'='*60}")
    print(f"WTI_N6 Crude Oil â€” Cycle at {ct.strftime('%Y-%m-%d %H:%M')} CT")
    ...
    try:
        ...
        _sl_today = sum(1 for _d in _deals_today
                        if _d.entry == 1 and 'WTI' in _d.symbol
    ...
    # ── MECHANICAL-PRIMARY + LLM-VETO RESILIENTE (2026-06-09) ────────────────
    # La señal mecánica (trend EMA20/100, PF 1.31) se computa SIEMPRE, independiente
    # del LLM. El LLM se intenta como veto; si CAE (sin créditos/timeout), se toma la
    # mecánica igual (modo resiliencia) — el portfolio NO queda ciego ante cortes API.
    from portfolio_guard import mechanical_signal as _mech, resilient_decision as _rdec
    from config import calc_lots as _calc_lots
    from config import EMA_FAST as _EF, EMA_SLOW as _ES
    from config import SL_ATR_MULT, TP_ATR_MULT   # ⚠️ sin esto: NameError silencioso (visto 06-10)
    _m = _mech(bars, mode="trend", sl_atr=SL_ATR_MULT, tp_atr=TP_ATR_MULT, ema_fast=_EF, ema_slow=_ES)

    _llm_dir, _llm_conf, _llm_failed = None, "LOW", False
    trend = {}
    if _m and _llm_capped:
        # Cuota LLM agotada: tomar mecánica sin veto (tratado igual que LLM caído).
        _llm_failed = True
        print(f"[RESILIENCIA] cuota LLM agotada -> tomo mecánica sin veto")
    elif _m:  # solo gastar LLM si hay señal mecánica que vetar
        try:
            trend = call_trend_agent(summary, fund_ctx)
            risk  = call_risk_agent(summary, trend, fund_ctx)
            if "error" in trend or "error" in risk:
                raise RuntimeError(trend.get("error") or risk.get("error"))
            _ls = synthesize_signal(trend, risk)
            _llm_dir = _ls.get("signal", "FLAT"); _llm_conf = _ls.get("confidence", "LOW")
        except Exception as _exc:
            _llm_failed = True
            _log("agent_error", {"error": str(_exc)[:120], "fallback": "mecánico sin veto"})
            print(f"[RESILIENCIA] LLM caído ({str(_exc)[:50]}) -> tomo mecánica sin veto")
            trend = {}

    _dec, _why = _rdec(_m, _llm_dir, _llm_conf, _llm_failed)
    if _dec == "TAKE":
        _stop_pts = abs(_m["entry"] - _m["sl"]); _lots = _calc_lots(_stop_pts)
        signal = {"signal": _m["signal"], "entry": _m["entry"], "sl": _m["sl"], "tp": _m["tp"],
                  "lots": _lots, "rr": _m["rr"], "risk_usd": round(_stop_pts*_lots, 2),
                  "confidence": "MEDIUM", "oil_regime": (trend.get("oil_regime","") if isinstance(trend,dict) else ""),
                  "opec_dynamic": "", "reasoning": f"{_m['basis']} | {_why}"}
        if _llm_failed: _log("resilient_take", {"signal": _m["signal"], "reason": _why})
    else:
        if _llm_dir and _llm_dir != (_m or {}).get("signal"):
            _log("llm_veto", {"mech": (_m or {}).get("signal"), "llm": _llm_dir, "conf": _llm_conf})
        signal = {"signal": "FLAT", "confidence": "LOW", "reasoning": _why}
    ...
    if signal["signal"] != "FLAT":
        ...
        elif _is_counter and _conf == "HIGH":
            print(f"[TREND FILTER] {_sig} contra {_oil_reg} PERMITIDO (conf=HIGH)")
            _log("trend_filter_allow", {"signal":_sig,"oil_regime":_oil_reg,"confidence":_conf})
    # -------------------------------------------------------------------------
    ...
    if execute and signal["signal"] != "FLAT":
        ...
        # SIN TP (validado 2026-06-10, validate_tp_extension/pyramiding): la salida ganadora
        # la decide el trailing 1.5R desde +2R en manage_position_fast (expR +0.28→+0.46).
        # El SL siempre vive en el servidor; el TP fijo amputaba la cola derecha (post-EIA).
        # NOTA: place_order NO acepta 'deviation' (TypeError silencioso que bloqueó
        # ejecuciones). Ademas ORDER_DEVIATION es no-op: broker usa MARKET execution.
        result = place_order(
            direction=mt5_dir, symbol=SYMBOL,
            volume=signal.get("lots", DEFAULT_VOLUME),
            sl=_sl, tp=None,
            comment="WTI_QA", magic=OUR_MAGIC, dry_run=False,
        )
        _log("execution",{"result":result,"direction":mt5_dir,"volume":signal.get("lots"),
                          "fill_price":result.get("price"),"ok":result.get("ok")})
        if result.get("ok"):
            print(f"    ticket={result.get('order')} price={result.get('price')} (sin TP — trailing manda)")
            # Persistir referencia ORIGINAL del trade para la gestion (R no se puede
            # derivar del SL una vez movido a breakeven).
            _fill = float(result.get("price") or signal.get("entry") or 0)
            _r0   = abs(_fill - _sl) if (_sl and _fill) else None
            state["open_trade"] = {"entry": _fill, "r_unit": _r0, "dir": signal["signal"],
                                   "lots": signal.get("lots"), "ticket": result.get("order"),
                                   "pyramided": False, "ts": signal["timestamp"]}
            _save_state(state)
        else:
...
def manage_position_fast(execute=False):
    ...
    try:
        ...
        state = _load_state()
        if not _ours:
            if state.get("open_trade"):           # quedo flat -> limpiar referencia
                state.pop("open_trade", None); _save_state(state)
            return False

        # ── Gestion WTI validada 2026-06-10 (validate_pyramiding/tp_extension):
        #    BE@1R + add 0.5x al +1R (SL->entry) + SIN TP + trail 1.5R desde +2R.
        #    expR +0.28 -> +0.46, worst sigue -1R, walk-forward 1.78 consist 75%.
        ot = state.get("open_trade") or {}
        _p0    = _ours[0]
        entry  = float(ot.get("entry") or _p0["price_open"])
        is_long = (ot.get("dir") == "LONG") if ot.get("dir") else (_p0["type"] == 0)
        sgn    = 1 if is_long else -1
        cur    = float(_p0["price_current"])
        r0     = ot.get("r_unit")
        if not r0 or r0 <= 0:
            ...
            r0 = _atr * SL_ATR_MULT if _atr else 0
        if r0 <= 0:
            return True
        profit_r = sgn * (cur - entry) / r0

        def _ratchet(level):
            n = 0
            for _u in _ours:
                _better = level > _u.get("sl", 0) if is_long else (not _u.get("sl") or level < _u["sl"])
                if _better and _msl(_u["ticket"], round(level, 5)):
                    n += 1
            return n

        # 1) Trailing 1.5R desde +2R (prioridad: ya protege mas que el BE)
        if profit_r >= 2.0:
            _cand = cur - sgn * 1.5 * r0
            _n = _ratchet(_cand)
            if _n:
                print(f"[FAST MGMT] trail {profit_r:.2f}R -> SL {_cand:.3f} ({_n} pos)")
                _log("fast_mgmt", {"action": "trail", "profit_r": round(profit_r, 2),
                                   "new_sl": round(_cand, 3), "positions": _n})
        # 2) Breakeven al +1R
        elif profit_r >= 1.0:
            _be = entry + sgn * 0.05 * r0
            _n = _ratchet(_be)
            if _n:
                print(f"[FAST MGMT] breakeven {profit_r:.2f}R -> SL {_be:.3f} ({_n} pos)")
                _log("fast_mgmt", {"action": "breakeven", "profit_r": round(profit_r, 2),
                                   "new_sl": round(_be, 3), "positions": _n})

        # 3) Pyramid al +1R + PROTECT_TR (validado walk-forward 2026-06-24: mismo expR que
        #    'current' +0.291 pero giveback 33%->0% y peor-fold -0.69->-0.13). Al agregar la
        #    unit2, su SL queda en SU PROPIA entrada (BE) y el SL de la unit1 se mueve a la
        #    entrada de la unit2 => lockea el +1R de la lectura buena (no se barre). Ambas
        #    siguen trailando juntas desde +2R via _ratchet.
        if execute and profit_r >= 1.0 and not ot.get("pyramided") and len(_ours) == 1 and ot.get("lots"):
            _add_lots = max(0.1, round(float(ot["lots"]) * 0.5, 1))
            _res = place_order(direction="BUY" if is_long else "SELL", symbol=SYMBOL,
                               volume=_add_lots, sl=round(cur, 3), tp=None,
                               comment="WTI_PYR", magic=OUR_MAGIC, dry_run=False)
            _log("pyramid_add", {"ok": _res.get("ok"), "lots": _add_lots,
                                 "price": _res.get("price"), "profit_r": round(profit_r, 2)})
            if _res.get("ok"):
                _e2 = _res.get("price") or cur     # entrada real de la unit2
                _msl(_ours[0]["ticket"], round(_e2, 3))   # PROTECT: SL de u1 -> entrada de u2 (lockea +1R)
                print(f"[FAST MGMT] PYRAMID+PROTECT add {_add_lots} @ {_e2} (+{profit_r:.2f}R) | "
                      f"SL u1 -> {_e2:.3f} (lockea +1R), u2 BE")
                _log("pyramid_protect", {"u1_ticket": _ours[0]["ticket"], "u2_entry": round(_e2, 3),
                                         "locked_r": round(profit_r, 2)})
                ot["pyramided"] = True
                state["open_trade"] = ot
                _save_state(state)
        return True
        ...
        try: _log("mgmt_error", {"error": str(_fe)[:150]})
        except Exception: pass
        return False
