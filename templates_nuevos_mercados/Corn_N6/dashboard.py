"""Corn_N6 — Web Dashboard (port 8089)"""
...
def _build_html():
    ...
    for t in reversed(trades[-20:]):
        ...
        reg = t.get("oil_regime","?"); ext = t.get("opec_dynamic","?")
        rows += f"""<tr>
          <td>{ts}</td>
...
<title>{EMOJI} Corn_N6 Dashboard</title>
<style>
...
<div class="sig-card">
  ...
  | Régimen: {sig.get("oil_regime","—")} | Contexto: {sig.get("opec_dynamic","—")}</small><br>
...
def start_dashboard(port=PORT):
    ...
    print(f"Corn_N6 dashboard: http://localhost:{port}")
    srv.serve_forever()
