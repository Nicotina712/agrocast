...
# Motivo: sin edge mecánico (WF trend −0.27), granos CFD mueren por spread, $0 en P&L. NO quitar sin re-evaluar.
print("Corn_N6 RETIRADO — runner deshabilitado (sin edge; granos CFD spread). Pendiente: research futuros materias primas.")
sys.exit(0)
...
from config import (
    ...
    MAX_ENTRY_SLIP_PCT,
    DEFAULT_VOLUME,
    SL_ATR_MULT, TP_ATR_MULT, calc_lots,
...
OUR_MAGIC    = 20260610   # Corn_N6 robot magic number
...
def _build_fundamental_context():
    ct = _now_ct()
    return {
        "usda_reports":   "unknown â€” WASDE / Crop Progress / Grain Stocks (~07:30 CT report days)",
        "weather":        "unknown â€” Midwest growing-season weather is the dominant catalyst",
        "export_sales":   "unknown â€” USDA export sales Thursday; China/Mexico demand",
        "seasonality":    "unknown â€” planting/pollination/harvest phase",
        "usd_index":      "unknown â€” strong USD pressures US grain export competitiveness",
        "ethanol_crude":  "unknown â€” corn feedstock, loose correlation with crude oil",
        "session_note":   f"Corn CBOT | CT: {ct.strftime('%H:%M')} | weekday: {ct.weekday()}",
        "key_levels_note":"Watch round numbers per bushel: 400, 425, 450, 475, 500 cents",
    }
...
def run_cycle(execute=False):
    ...
    print(f"\n{'='*60}")
    print(f"Corn_N6 CBOT Corn â€” Cycle at {ct.strftime('%Y-%m-%d %H:%M')} CT")
    ...
    # RESILIENCIA tope-diario: si se agotó la cuota LLM, NO ir dark — usar el
    # fallback mecánico (trend 20/50, validado +EV). Solo se marca _llm_capped.
    _llm_capped = not can_call
    if _llm_capped:
        print(f"LLM gate: {reason} -> fallback mecánico (resiliencia tope)")
        _log("llm_gate", {"reason": reason, "state_date": state.get("date"),
                          "llm_calls": state.get("llm_calls"), "fallback": "mecánico trend 20/50"})
    ...
    # â”€â”€ LLM-PRIMARY con FALLBACK MECANICO (resiliencia ante corte de creditos API) â”€â”€
    # Normal: señal del LLM. Si el LLM CAE, se usa la mecánica trend EMA20/50
    # (validada robusta walk-forward: meanPF 1.26, consist 75%, minPF 0.73 — = config).
    _llm_failed = False
    trend, risk = {}, {}
    try:
        if _llm_capped:
            raise RuntimeError("cuota LLM agotada (tope diario)")
        print("Calling Trend Agent (Corn)...")
        trend = call_trend_agent(summary, fund_ctx)
        if "error" in trend:
            raise RuntimeError(trend["error"])
        setup = trend.get("setup",{})
        print(f"Trend: {trend.get('trend')} {trend.get('trend_strength')} | Regime: {trend.get('oil_regime')} | {setup.get('type')} -> {setup.get('direction')} ({setup.get('confidence')})")
        print("Calling Risk Agent (Corn)...")
        ...
        _llm_failed = True
        _log("agent_error", {"error": str(_exc)[:120], "fallback": "mecánico trend 20/50"})
        print(f"[RESILIENCIA] LLM caído ({str(_exc)[:50]}) -> fallback mecánico trend 20/50")
        from portfolio_guard import mechanical_signal as _mech
        _m = _mech(bars, mode="trend", sl_atr=SL_ATR_MULT, tp_atr=TP_ATR_MULT, ema_fast=20, ema_slow=50)
        if _m:
            _stop_pts = abs(_m["entry"] - _m["sl"]); _lots = calc_lots(_stop_pts)
            signal = {"signal": _m["signal"], "entry": _m["entry"], "sl": _m["sl"], "tp": _m["tp"],
                      "lots": _lots, "rr": _m["rr"], "confidence": "MEDIUM",
                      "reasoning": f"FALLBACK mecánico (LLM caído): {_m['basis']}"}
            _log("resilient_take", {"signal": _m["signal"], "basis": _m["basis"]})
            print(f"[RESILIENCIA] Mecánica toma {_m['signal']} @ {_m['entry']}")
        else:
            signal = {"signal": "FLAT", "confidence": "LOW", "reasoning": "FALLBACK mecánico FLAT (sin señal)"}
    ...
    if execute and signal["signal"] != "FLAT":
        ...
        result = place_order(
            ...
            sl=_sl, tp=_tp,
            comment="CORN_QA", magic=OUR_MAGIC, dry_run=False,
        )
...
def main():
    ...
    if execute:
        print("LIVE EXECUTION ENABLED (Corn demo)")

    if args.loop:
        print(f"Corn loop. Cycle: {CYCLE_MINUTES} min. Prime: {_PRIME_START[0]:02d}:{_PRIME_START[1]:02d}-{_PRIME_END[0]:02d}:{_PRIME_END[1]:02d} CT (Mon-Fri)")
        os.makedirs(ARTIFACTS_DIR, exist_ok=True)
        ...
        while True:
            try:
                if _is_trading_day():
                    ...
                    print("Weekend â€” WTI closed. Skip.")
