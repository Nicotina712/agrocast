...
OUR_MAGIC = 20260604   # US500 robot magic number
...
def _build_fundamental_context():
    ...
    return {
        ...
        "fed_policy":          "unknown â€” check FOMC minutes/dots",
        "sp500_sector_rotation": "unknown â€” check sector ETFs",
        "mag7_earnings":       "unknown â€” AAPL/NVDA/MSFT/GOOGL/META/AMZN/TSLA earnings",
        "us_macro":            "unknown â€” NFP, CPI, GDP",
        "dxy_trend":           "unknown â€” DXY inverse correlation",
        "session_note":        f"US500 market hours | CT: {ct.strftime('%H:%M')} | weekday: {ct.weekday()}",
        "key_levels_note":     "Watch: 4800, 5000, 5200, 5500, 5800, 6000 round levels",
    }
...
def run_cycle(execute=False):
    ...
    print(f"\n{'='*60}")
    print(f"US500 S&P 500 â€” Cycle at {ct.strftime('%Y-%m-%d %H:%M')} CT")
    print(f"Prime: {is_prime_now} | Weekend: {is_wknd} | Mins left: {mins_left}")
    print("="*60)

    if not _is_trading_day():
        print("Weekend â€” US500 market closed. Skip.")
    ...
    # fallback mecánico (trend 20/100, validado +EV). Solo se marca _llm_capped.
    ...
    if _llm_capped:
        ...
        _log("llm_gate", {"reason": reason, "state_date": state.get("date"),
                          "llm_calls": state.get("llm_calls"), "fallback": "mecánico trend 20/100"})
    ...
    # Normal: señal del LLM. Si el LLM CAE, se usa la mecánica trend EMA20/100
    # (validada robusta walk-forward: meanPF 1.42, consist 88% — NO la config 10/50).
    # ── MECHANICAL-PRIMARY (VWAP mean-reversion 1h) + LLM-VETO GATEADO (migrado 2026-06-26) ──
    # US500 era LLM-primary (operaba casi nada). El S&P MEAN-REVIERTE en sesión cash (como US30/Dow).
    # WF 8 folds: VWAP-rev 1h th0.3 SL2.0/TP4.0 -> meanPF 2.05, minPF 0.89, consist 75%, expR +0.59, N111.
    # La señal de reversión MANDA; el LLM solo VETA si ve tendencia opuesta FUERTE (HIGH). GATEADO.
    _llm_failed = False
    from portfolio_guard import mechanical_signal as _mech
    from config import VWAP_TH as _VWAP_TH, SL_ATR_MULT as _SLM, TP_ATR_MULT as _TPM, calc_lots as _calc_lots
    _m = _mech(bars, mode="vwap_reversion", sl_atr=_SLM, tp_atr=_TPM, vwap_th=_VWAP_TH)
    _llm_dir, _llm_conf = "FLAT", "LOW"
    trend, risk = {}, {}
    if _m and not _llm_capped:
        try:
            print("Calling Trend+Risk Agent (US500, veto)...")
    ...
    if not _m:
        ...
        print(f"[LLM VETO] LLM ve {_llm_dir} HIGH contra mean-rev {_m['signal']} -> FLAT")
    ...
    if execute and signal["signal"] != "FLAT":
        ...
        result = place_order(
            ...
            sl=_sl, tp=_tp,
            comment="US500_QA", magic=OUR_MAGIC, dry_run=False,
        )
...
def main():
    ...
    if execute:
        print("LIVE EXECUTION ENABLED (US500 demo)")

    if args.loop:
        print(f"US500 loop. Cycle: {CYCLE_MINUTES} min. Prime: {_PRIME_START[0]:02d}:{_PRIME_START[1]:02d}-{_PRIME_END[0]:02d}:{_PRIME_END[1]:02d} CT (Mon-Fri)")
        os.makedirs(ARTIFACTS_DIR, exist_ok=True)
        ...
        while True:
            try:
                if _is_trading_day():
                    ...
                    print("Weekend â€” US500 closed. Skip.")
