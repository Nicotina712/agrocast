"""US500 S&P 500 Robot — Execution Tracker"""
...
def compute_performance(trades):
    ...
    regimes = {}
    for t in trades:
        r = t.get("sp500_regime","unknown"); regimes[r] = regimes.get(r,0)+1
    return {"total_trades":len(trades),"longs":len(longs),"shorts":len(shorts),
            "avg_rr":avg_rr,"avg_risk_usd":avg_risk,"confidence_dist":conf_dist,"sp500_regime_dist":regimes}
...
def print_report():
    ...
    print("\n" + "="*58)
    print("  US500 S&P 500 Robot — Performance Report")
    print("="*58)
    ...
    print(f"  S&P Regime dist    : {stats.get('sp500_regime_dist',{})}")
    ...
    if trades:
        ...
        for t in trades[-5:]:
            ...
            print(f"  {ts} | {sig:5s} @ {ent_s:>12} | SL:{sl_s:>12} | TP:{tp_s:>12} | R:R {t.get('rr','?')} | {t.get('confidence','?')} | {t.get('sp500_regime','?')}")


def main():
    parser = argparse.ArgumentParser(description="US500 Execution Tracker")
    parser.add_argument("--report", action="store_true")
    parser.add_argument("--sync",   action="store_true")
    args = parser.parse_args()
    if args.sync:
        fills = sync_fills()
        print(f"Open US500 positions (magic={OUR_MAGIC}): {len(fills)}")
