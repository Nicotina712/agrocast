"""
ETHUSD Ethereum Robot — Microstructure Features
Adapts the feature pipeline for Ethereum CFD.

ETH differences vs BTC:
  - Lower absolute price (~$2,100) but higher relative volatility (beta ~1.5× BTC)
  - ATR 5m: typically $8-60 depending on session
  - Same 24/7 structure, midnight UTC VWAP reset
  - Key round numbers: $1.8k, $2k, $2.2k, $2.5k, $3k (every $500)
  - ETH/BTC ratio itself is a critical feature (not computed here, added to fundamental context)
  - Volume column: "volume" (same as BTC CFD)
"""
...
def build_intraday_features(df_bars: pd.DataFrame, interval: str = "5m") -> pd.DataFrame:
    ...
    # ETH-specific: distance to round numbers ($500 steps)
    round_500  = (close / 500).round() * 500
    feats["dist_round500"]  = (close - round_500) / 500
    round_1000 = (close / 1000).round() * 1000
    feats["dist_round1k"]   = (close - round_1000) / 1000
    # Reuse field name from BTCUSD for compatibility
    feats["dist_round5k"]   = feats["dist_round500"]
...
def summarize_bars(df_bars: pd.DataFrame, df_feats: pd.DataFrame) -> dict:
    ...
    for i in range(min(6, len(df_bars))):
        b = df_bars.iloc[-(i+1)]
        f = df_feats.iloc[-(i+1)]
        vol_val = int(b["volume"] if "volume" in b.index else b.get("tick_volume", 0))
        recent_bars.append({
            "i":     -i,
    ...
    return {
        ...
        "indicators": {
            ...
            "dist_round500":  round(float(latest.get("dist_round500", 0))*100, 2),
        },
        "returns": {
