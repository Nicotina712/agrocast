"""
STOXX50 Euro Stoxx 50 Index Robot — AI Agents
Two-agent system specialized for STOXX50 (Euro Stoxx 50) CFD intraday trading.
...
STOXX50 / Euro Stoxx 50 fundamentals that matter:
...
TREND_AGENT_SYSTEM = """You are a professional Euro Stoxx 50 Index (STOXX50) intraday trader and technical analyst.
You specialize in 30-minute chart analysis during the Hong Kong cash session (HKT 09:30-16:00, lunch 12:00-13:00).

Your expertise:
- STOXX50 intraday momentum and trend patterns on 30m charts
...
RISK_AGENT_SYSTEM = """You are a professional risk manager for Euro Stoxx 50 Index (STOXX50) intraday trading.
You specialize in position sizing and trade management for STOXX50 CFDs.

Account parameters:
- Capital: $1,013 USD (demo account, ICMarkets)
- Max risk per trade: 3% = ~$30.39 USD
- Instrument: STOXX50 CFD (≈ $1/pt P&L on demo)
...
STOXX50-specific risk considerations:
...
def call_trend_agent(bars_summary: dict, fundamental_ctx: dict, model_id: str = MODEL_ID) -> dict:
    user_msg = f"""Analyze STOXX50 (Euro Stoxx 50) 30-minute chart for intraday trading signal.
...
def call_risk_agent(bars_summary: dict, trend_analysis: dict, fundamental_ctx: dict, model_id: str = MODEL_ID) -> dict:
    ...
    user_msg = f"""Evaluate risk and size a STOXX50 trade based on trend analysis.
