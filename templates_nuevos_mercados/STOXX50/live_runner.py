...
OUR_MAGIC = 20260614   # STOXX50   # HK50 robot magic number
...
def _build_fundamental_context():
    ...
    return {
        ...
        "session_note":        f"STOXX50 (HKT) | local: {ct.strftime('%H:%M')} | weekday: {ct.weekday()}",
...
def run_cycle(execute=False):
    ...
    print(f"\n{'='*60}")
    print(f"STOXX50 â€” Cycle at {ct.strftime('%Y-%m-%d %H:%M')} HKT")
    ...
    if _llm_capped or _msig0 is None:
        ...
        print("Calling Trend+Risk Agent (STOXX50, veto)...")
        try:
    ...
    # ── FILTRO DE CONTEXTO (validado 2026-06-14, Δ+0.17 93% folds, placebo ~0): el fade VWAP de
    # STOXX50 rinde mejor fadeando EN LA DIRECCION del momentum de DE40 (DAX, proxy europeo natural).
    if _m:
        try:
            from portfolio_guard import context_aligned as _ctxa
            _al = _ctxa("DE40", _m["signal"])
            if _al is False:
                print(f"[CTX FILTER] fade {_m['signal']} contra momentum DAX (DE40) -> skip")
                _log("ctx_filter_skip", {"mech": _m["signal"], "proxy": "DE40"}); _m = None
        except Exception as _ce:
            _log("ctx_filter_err", {"error": str(_ce)[:100]})
    if not _m:
        signal = {"signal": "FLAT", "confidence": "LOW", "reasoning": "sin extension VWAP / contra-DAX"}
    ...
    if execute and signal["signal"] != "FLAT":
        ...
        result = place_order(
            ...
            sl=_sl, tp=_tp,
            comment="STOXX50_QA", magic=OUR_MAGIC, dry_run=False,
        )
