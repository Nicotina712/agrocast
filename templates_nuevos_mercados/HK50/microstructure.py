"""
US500 S&P 500 Robot — Microstructure Features
Adapts the feature pipeline for S&P 500 CFD.

US500 differences vs crypto:
  - Trades only Mon-Fri during market hours (09:30-16:00 ET)
  - ATR 5m: typically 5-20 pts depending on session
  - Daily VWAP reset at market open (midnight UTC normalize works fine)
  - Key round numbers: every 100pts and 500pts (4800, 5000, 5200, 5500, 5800, 6000)
  - Volume column: tick_volume (MT5 returns tick_volume for indices, not volume)
  - Opening range (first 30min) is a critical reference
"""
...
def build_intraday_features(df_bars: pd.DataFrame, interval: str = "5m") -> pd.DataFrame:
    ...
    # US500-specific: distance to round numbers (100pt and 500pt steps)
    round_100  = (close / 100).round() * 100
    feats["dist_round100"]  = (close - round_100) / 100
    round_500  = (close / 500).round() * 500
    feats["dist_round500"]  = (close - round_500) / 500
    # Compatibility field name used by ML pipeline
    feats["dist_round5k"]   = feats["dist_round100"]
...
def summarize_bars(df_bars: pd.DataFrame, df_feats: pd.DataFrame) -> dict:
    ...
    return {
        ...
        "indicators": {
            ...
            "atr14_pts":      round(float(latest.get("atr14", 0)),  2),
            ...
            "dist_round100":  round(float(latest.get("dist_round100", 0))*100, 2),
