"""
HK50 Hang Seng — Live Runner — ⛔ RETIRADO 2026-06-10 (reemplazado por CHINA50).
GUARD: robot retirado. Sale de inmediato sin operar, sin importar quién lo lance
(bat/watchdog/scheduler) — incidente zombies recurrentes 06-13/14. NO quitar este guard.
"""
import sys as _sys
print("HK50 RETIRADO — runner deshabilitado (guard anti-zombie 2026-06-14)")
_sys.exit(0)
...
OUR_MAGIC = 20260611   # HK50 robot magic number
...
def _build_fundamental_context():
    ...
    return {
        ...
        "session_note":        f"HK50 Hang Seng (HKT) | local: {ct.strftime('%H:%M')} | weekday: {ct.weekday()}",
...
def run_cycle(execute=False):
    ...
    print(f"\n{'='*60}")
    print(f"HK50 Hang Seng â€” Cycle at {ct.strftime('%Y-%m-%d %H:%M')} HKT")
    ...
    if _llm_capped:
        ...
        print("Calling Trend+Risk Agent (HK50, veto)...")
        try:
    ...
    _m = _mech(bars, mode="vwap_reversion", sl_atr=SL_ATR_MULT, tp_atr=TP_ATR_MULT, vwap_th=0.5)
    if not _m:
    ...
    if execute and signal["signal"] != "FLAT":
        ...
        result = place_order(
            ...
            sl=_sl, tp=_tp,
            comment="HK50_QA", magic=OUR_MAGIC, dry_run=False,
        )
