...
def _build_html():
    ...
    for t in reversed(trades[-20:]):
        ...
        reg = t.get("uk100_regime","?"); ext = t.get("gbp_impact","?")
        rows += f"""<tr>
          <td>{ts}</td>
...
<title>{EMOJI} UK100 Dashboard</title>
<style>
...
<div class="sig-card">
  ...
  | Régimen: {sig.get("uk100_regime","—")} | Contexto: {sig.get("gbp_impact","—")}</small><br>
...
def start_dashboard(port=PORT):
    ...
    print(f"UK100 dashboard: http://localhost:{port}")
    srv.serve_forever()
