"""
UK100 FTSE 100 â€” Live Runner
Mon-Fri, prime session 02:00-11:30 CT (London hours).
...
def _log_paper(sig, summary):
    ...
    entry = {
        ...
        "uk100_regime":sig.get("uk100_regime"),"gbp_impact":sig.get("gbp_impact"),
...
def _build_fundamental_context():
    ct = _now_ct()
    return {
        "boe_policy":    "unknown â€” Bank of England rates (8 meetings/year)",
        "gbp_usd":       "unknown â€” FTSE inversely correlated with GBP strength",
        "uk_macro":      "unknown â€” UK CPI, GDP, PMI, retail sales",
        "oil_energy":    "unknown â€” BP/Shell ~15% of FTSE, oil price matters",
        "ftse_sectors":  "unknown â€” financials, energy, mining, pharma",
        "global_risk":   "unknown â€” FTSE100 multinational, tracks global growth",
        "session_note":  f"UK100 London session | CT: {ct.strftime('%H:%M')} | weekday: {ct.weekday()}",
        "key_levels_note":"Watch: 7500, 8000, 8500, 9000 psychological levels",
    }
...
def run_cycle(execute=False):
    ...
    print(f"\n{'='*60}")
    print(f"UK100 FTSE 100 â€” Cycle at {ct.strftime('%Y-%m-%d %H:%M')} CT")
    print(f"Prime: {is_prime_now} | Weekend: {is_wknd} | Mins left: {mins_left}")
    print("="*60)

    if not _is_trading_day():
        print("Weekend â€” UK100 market closed. Skip.")
    ...
    # WEEKEND PROFIT-CLOSE (06-29): viernes cerca del cierre semanal, si la posicion esta
    # en GANANCIA, cerrarla para no exponerla al gap del lunes (atraviesa el SL). Solo ganadores.
    ...
    try:
        ...
        _open = [p for p in _all_pos if p.get("magic") == OUR_MAGIC]  # any position on this symbol = block
        if _open:
            _p = _open[0]
            ...
            try:
                ...
                if _news_chk["exit"]:
                    _log("news_exit_triggered", {
                        "ticket": _ticket, "position_type": _ptype, "profit": _profit,
                        "news_score": _news_chk["score"], "reason": _news_chk["reason"][:200],
                        "n_conflicts": len(_news_chk["conflicts"]),
                    })
                    print(f"[NEWS MONITOR] >>> CIERRE ANTICIPADO por conflicto de noticias <<<")
                    _close_res = close_position(
                        ticket=_ticket, symbol=SYMBOL,
                        comment="news_exit", magic=OUR_MAGIC,
                        dry_run=not execute,
                    )
            ...
            # -- Fin news conflict check ----------------------------------------

            # ── BE@1R (validado 2026-06-10): mover SL a entry al +1R para que un
            # ganador que no llega a TP no se convierta en perdedor (UK100 +0.26→+0.28).
            try:
        ...
        print(f"[POSITION GUARD] check failed: {_pg_err} -- skipping cycle (conservative)")
    ...
    # ── MECHANICAL-PRIMARY (VWAP mean-reversion) + LLM-VETO + RESILIENCIA (2026-06-09)
    # UK100/FTSE es MEAN-REVERTING, no trending. 5m-trend perdia (walk-forward meanPF 1.03,
    # expR neg). VWAP-rev 30m th0.8 SL2/TP4 validado (meanPF 2.88, consist 88%, minPF 0.94).
    # La señal de reversion manda; el LLM solo VETA si propone dirección opuesta con conf HIGH.
    # Resiliencia: si el LLM cae o se agota la cuota, se toma la mecánica sin veto.
    from portfolio_guard import mechanical_signal as _mgate0
    from config import SL_ATR_MULT as _gsm, TP_ATR_MULT as _gtm, VWAP_TH as _gvt
    try: _msig0 = _mgate0(bars, mode="vwap_reversion", sl_atr=_gsm, tp_atr=_gtm, vwap_th=_gvt)
    ...
    if _llm_capped or _msig0 is None:
        ...
        try:
            print("Calling Trend+Risk Agent (UK100, veto)...")
    ...
    from config import calc_lots as _calc_lots, VWAP_TH as _VWAP_TH
    _m = _mech(bars, mode="vwap_reversion", sl_atr=SL_ATR_MULT, tp_atr=TP_ATR_MULT, vwap_th=_VWAP_TH)
    if not _m:
        signal = {"signal": "FLAT", "confidence": "LOW", "reasoning": "sin extension VWAP"}
    ...
    _save_state(state)
    _save_signal(signal)
    _log("signal",{k:signal.get(k) for k in ("signal","entry","sl","tp","lots","confidence","rr","risk_usd","uk100_regime")})
    ...
    if signal["signal"] != "FLAT":
        ...
        print(f"    Regime: {signal.get('uk100_regime')} | GBP: {signal.get('gbp_impact')} | Conf: {signal.get('confidence')}")
    else:
    ...
    if execute and signal["signal"] != "FLAT":
        ...
        result = place_order(
            ...
            sl=_sl, tp=_tp,
            comment="UK100_QA", magic=OUR_MAGIC, dry_run=False,
        )
...
def main():
    ...
    if execute:
        print("LIVE EXECUTION ENABLED (UK100 demo)")

    if args.loop:
        print(f"UK100 loop. Cycle: {CYCLE_MINUTES} min. Prime: {_PRIME_START[0]:02d}:{_PRIME_START[1]:02d}-{_PRIME_END[0]:02d}:{_PRIME_END[1]:02d} CT (Mon-Fri)")
        os.makedirs(ARTIFACTS_DIR, exist_ok=True)
        ...
        while True:
            try:
                if _is_trading_day():
                    ...
                    print("Weekend â€” UK100 closed. Skip.")
