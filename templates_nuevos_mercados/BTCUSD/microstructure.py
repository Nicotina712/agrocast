"""
BTCUSD Bitcoin Robot — Microstructure Features
Adapts the base microstructure module for Bitcoin CFD trading.

Key differences from Gold:
  - 24/7 market: no RTH window — we use a "prime session" concept
  - Bitcoin ATR is typically $300–2,000+ on 5-min bars (vs ~$3-10 for Gold)
  - Volume profile: lower on weekends, highest during NY session
  - No actual VWAP reset on open (24/7), but we reset at midnight UTC
  - Key BTC patterns: V-bottom at round numbers, CMC reversals
"""

import os
import sys
import numpy as np
import pandas as pd
from datetime import time, datetime, timezone, timedelta

_HERE = os.path.dirname(os.path.abspath(__file__))
_MVP_ROOT = os.path.dirname(os.path.dirname(_HERE))
sys.path.insert(0, _MVP_ROOT)

from config import CT_OFFSET_HOURS, PRIME_OPEN_CT, PRIME_CLOSE_CT

# Session hours for BTC "prime" (07:00-22:00 CT)
_PRIME_OPEN_CT  = PRIME_OPEN_CT
_PRIME_CLOSE_CT = PRIME_CLOSE_CT


def _utc_to_ct(ts_utc: pd.Timestamp) -> pd.Timestamp:
    return ts_utc + pd.Timedelta(hours=CT_OFFSET_HOURS)


def _is_prime_session(ts_utc: pd.Timestamp) -> bool:
    ct = _utc_to_ct(ts_utc)
    t  = ct.hour * 60 + ct.minute
    s  = _PRIME_OPEN_CT.hour  * 60 + _PRIME_OPEN_CT.minute
    e  = _PRIME_CLOSE_CT.hour * 60 + _PRIME_CLOSE_CT.minute
    return s <= t <= e


def _midnight_vwap(df: pd.DataFrame) -> pd.Series:
    """
    VWAP resets at midnight UTC for Bitcoin.
    Typical proxy: cumsum(price*vol) / cumsum(vol) within UTC day.
    """
    df = df.copy()
    df["_date_utc"] = df.index.normalize()
    typical = (df["high"] + df["low"] + df["close"]) / 3.0
    vol_col = "volume" if "volume" in df.columns else "tick_volume"
    df["_tp_vol"] = typical * df[vol_col]

    def _group_vwap(g):
        cumvol = g[vol_col].cumsum()
        cumtpv = g["_tp_vol"].cumsum()
        return cumtpv / cumvol.replace(0, np.nan)

    return df.groupby("_date_utc", group_keys=False).apply(_group_vwap)


def build_intraday_features(df_bars: pd.DataFrame, interval: str = "5m") -> pd.DataFrame:
    """
    Build 36 intraday features for BTCUSD from OHLCV bars.
    Returns DataFrame aligned with df_bars.
    """
    df = df_bars.copy()
    feats = pd.DataFrame(index=df.index)

    close = df["close"]
    high  = df["high"]
    low   = df["low"]
    # BTCUSD uses "volume" column; Gold uses "tick_volume" — handle both
    vol   = df["volume"] if "volume" in df.columns else df.get("tick_volume", pd.Series(1, index=df.index))

    # ── 1. Price returns ──────────────────────────────────────────────────────
    feats["ret_1"]  = close.pct_change(1)
    feats["ret_3"]  = close.pct_change(3)
    feats["ret_6"]  = close.pct_change(6)
    feats["ret_12"] = close.pct_change(12)

    # ── 2. Trend EMAs ─────────────────────────────────────────────────────────
    feats["ema9"]  = close.ewm(span=9,  adjust=False).mean()
    feats["ema21"] = close.ewm(span=21, adjust=False).mean()
    feats["ema50"] = close.ewm(span=50, adjust=False).mean()
    feats["ema9_cross_21"] = (feats["ema9"] - feats["ema21"]) / close

    # ── 3. RSI ────────────────────────────────────────────────────────────────
    delta = close.diff()
    gain  = delta.clip(lower=0).ewm(span=14, adjust=False).mean()
    loss  = (-delta.clip(upper=0)).ewm(span=14, adjust=False).mean()
    feats["rsi14"] = 100 - (100 / (1 + gain / loss.replace(0, np.nan)))

    # ── 4. ATR ────────────────────────────────────────────────────────────────
    tr = pd.concat([
        high - low,
        (high - close.shift(1)).abs(),
        (low  - close.shift(1)).abs(),
    ], axis=1).max(axis=1)
    feats["atr14"]      = tr.ewm(span=14, adjust=False).mean()
    feats["atr14_norm"] = feats["atr14"] / close  # normalized ATR

    # ── 5. VWAP (midnight reset) ──────────────────────────────────────────────
    try:
        feats["vwap"]      = _midnight_vwap(df)
        feats["vwap_dist"] = (close - feats["vwap"]) / feats["vwap"]
    except Exception:
        feats["vwap"]      = close.rolling(20).mean()
        feats["vwap_dist"] = (close - feats["vwap"]) / feats["vwap"]

    # ── 6. Bollinger Bands ────────────────────────────────────────────────────
    bb_mid = close.rolling(20).mean()
    bb_std = close.rolling(20).std()
    feats["bb_upper"] = bb_mid + 2 * bb_std
    feats["bb_lower"] = bb_mid - 2 * bb_std
    feats["bb_pct"]   = (close - feats["bb_lower"]) / (feats["bb_upper"] - feats["bb_lower"] + 1e-8)
    feats["bb_width"] = (feats["bb_upper"] - feats["bb_lower"]) / bb_mid

    # ── 7. Volume features ────────────────────────────────────────────────────
    feats["vol_ma20"]     = vol.rolling(20).mean()
    feats["vol_ratio"]    = vol / feats["vol_ma20"].replace(0, np.nan)
    feats["vol_trend"]    = vol.rolling(5).mean() / vol.rolling(20).mean()

    # ── 8. Candle structure ───────────────────────────────────────────────────
    candle_range = (high - low).replace(0, np.nan)
    feats["body_pct"]  = (close - df["open"]).abs() / candle_range
    feats["upper_wick"] = (high - df[["open", "close"]].max(axis=1)) / candle_range
    feats["lower_wick"] = (df[["open", "close"]].min(axis=1) - low) / candle_range
    feats["bull_candle"] = (close > df["open"]).astype(float)

    # ── 9. Rolling realized volatility ───────────────────────────────────────
    log_ret = np.log(close / close.shift(1))
    feats["realized_vol_12"] = log_ret.rolling(12).std() * np.sqrt(12)
    feats["realized_vol_24"] = log_ret.rolling(24).std() * np.sqrt(24)

    # ── 10. Momentum ──────────────────────────────────────────────────────────
    feats["mom_6"]  = close / close.shift(6)  - 1
    feats["mom_12"] = close / close.shift(12) - 1
    feats["mom_24"] = close / close.shift(24) - 1

    # ── 11. Cumulative delta proxy ────────────────────────────────────────────
    # Up/down volume proxy (no tick data available)
    up_vol   = vol * feats["bull_candle"]
    down_vol = vol * (1 - feats["bull_candle"])
    feats["cum_delta_12"] = (up_vol - down_vol).rolling(12).sum()
    feats["cum_delta_norm"] = feats["cum_delta_12"] / (vol.rolling(12).sum().replace(0, np.nan))

    # ── 12. Session features ──────────────────────────────────────────────────
    feats["is_prime_session"] = pd.Series(
        [_is_prime_session(ts) for ts in df.index], index=df.index
    ).astype(float)

    # CT hour (for agent context)
    feats["ct_hour"] = pd.Series(
        [_utc_to_ct(ts).hour for ts in df.index], index=df.index
    ).astype(float)

    feats["is_weekend"] = pd.Series(
        [_utc_to_ct(ts).weekday() >= 5 for ts in df.index], index=df.index
    ).astype(float)

    # Minutes from midnight UTC (position in day)
    feats["day_frac"] = pd.Series(
        [(ts.hour * 60 + ts.minute) / 1440.0 for ts in df.index], index=df.index
    )

    # ── 13. Distance to round numbers (BTC specific) ──────────────────────────
    # Nearest $5,000 multiple (e.g., $75k, $80k)
    round_5k  = (close / 5000).round() * 5000
    feats["dist_round5k"] = (close - round_5k) / 5000  # fraction of $5k
    # Nearest $10,000 multiple (e.g., $70k, $80k, $90k, $100k)
    round_10k = (close / 10000).round() * 10000
    feats["dist_round1k"] = (close - round_10k) / 10000  # reuse name for compat

    return feats.ffill().bfill()


def summarize_bars(df_bars: pd.DataFrame, df_feats: pd.DataFrame) -> dict:
    """
    Compress features into a LLM-friendly summary dict.
    """
    latest = df_feats.iloc[-1]
    bar    = df_bars.iloc[-1]

    # Recent price context
    lookback = min(24, len(df_bars))
    recent   = df_bars.iloc[-lookback:]

    current_price = float(bar["close"])
    session_high  = float(recent["high"].max())
    session_low   = float(recent["low"].min())

    # Convert ts to CT
    ts_utc = df_bars.index[-1]
    ts_ct  = _utc_to_ct(ts_utc)

    is_prime = bool(latest.get("is_prime_session", 0))
    is_wknd  = bool(latest.get("is_weekend", 0))

    mins_to_close = 0
    if is_prime:
        end_t = _PRIME_CLOSE_CT.hour * 60 + _PRIME_CLOSE_CT.minute
        now_t = ts_ct.hour * 60 + ts_ct.minute
        mins_to_close = max(0, end_t - now_t)

    # Recent bars (last 6 candles) for agent context
    recent_bars = []
    for i in range(min(6, len(df_bars))):
        b = df_bars.iloc[-(i+1)]
        f = df_feats.iloc[-(i+1)]
        recent_bars.append({
            "i":       -i,
            "open":    round(float(b["open"]),  2),
            "high":    round(float(b["high"]),  2),
            "low":     round(float(b["low"]),   2),
            "close":   round(float(b["close"]), 2),
            "vol":     int(b["volume"] if "volume" in b.index else b.get("tick_volume", 0)),
            "rsi":     round(float(f.get("rsi14", 50)), 1),
            "vwap_d":  round(float(f.get("vwap_dist", 0)) * 100, 3),  # % from VWAP
        })

    return {
        "symbol":       "BTCUSD",
        "current_state": {
            "close":         current_price,
            "ct_time":       ts_ct.strftime("%H:%M"),
            "is_prime":      is_prime,
            "is_weekend":    is_wknd,
            "mins_to_close": mins_to_close,
        },
        "session_range": {
            "high":  round(session_high, 2),
            "low":   round(session_low,  2),
            "range": round(session_high - session_low, 2),
        },
        "indicators": {
            "rsi14":            round(float(latest.get("rsi14", 50)), 1),
            "atr14_usd":        round(float(latest.get("atr14", 0)),  2),
            "atr14_pct":        round(float(latest.get("atr14_norm", 0)) * 100, 3),
            "vwap_dist_pct":    round(float(latest.get("vwap_dist", 0)) * 100, 3),
            "bb_pct":           round(float(latest.get("bb_pct", 0.5)), 3),
            "bb_width_pct":     round(float(latest.get("bb_width", 0)) * 100, 3),
            "ema9":             round(float(latest.get("ema9", current_price)), 2),
            "ema21":            round(float(latest.get("ema21", current_price)), 2),
            "ema50":            round(float(latest.get("ema50", current_price)), 2),
            "vol_ratio":        round(float(latest.get("vol_ratio", 1)), 2),
            "realized_vol_12":  round(float(latest.get("realized_vol_12", 0)) * 100, 3),
            "mom_6_pct":        round(float(latest.get("mom_6", 0)) * 100, 3),
            "mom_24_pct":       round(float(latest.get("mom_24", 0)) * 100, 3),
            "cum_delta_norm":   round(float(latest.get("cum_delta_norm", 0)), 3),
            "dist_round1k_pct": round(float(latest.get("dist_round1k", 0)) * 100, 2),
        },
        "returns": {
            "ret_1":  round(float(latest.get("ret_1",  0)) * 100, 4),
            "ret_3":  round(float(latest.get("ret_3",  0)) * 100, 4),
            "ret_6":  round(float(latest.get("ret_6",  0)) * 100, 4),
            "ret_12": round(float(latest.get("ret_12", 0)) * 100, 4),
        },
        "recent_bars": recent_bars,
    }
