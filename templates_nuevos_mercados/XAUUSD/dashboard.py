"""XAUUSD — Web Dashboard (port 8080)"""
...
EMOJI  = "🟡"
NAME   = "Gold / XAUUSD"
REG_FIELD = "volatility_regime"
CTX_FIELD = "market_context"
...
def _build_html():
    ...
    for t in reversed(trades[-20:]):
        ...
        reg = t.get(REG_FIELD, "—"); ctx = t.get(CTX_FIELD, "—")
        rows += f"""<tr>
          <td>{ts}</td>
...
<title>{EMOJI} XAUUSD Dashboard</title>
<style>
...
<div class="sig-card">
  ...
  | Régimen: {sig.get(REG_FIELD,"—")} | Contexto: {sig.get(CTX_FIELD,"—")}</small><br>
...
def start_dashboard(port=PORT):
    ...
    print(f"XAUUSD dashboard: http://localhost:{port}")
    srv.serve_forever()
