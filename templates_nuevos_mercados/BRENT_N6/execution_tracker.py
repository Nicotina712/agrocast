...
def compute_performance(trades):
    ...
    regimes = {}
    for t in trades:
        r = t.get("oil_regime","unknown"); regimes[r] = regimes.get(r,0)+1
    extra_dist = {}
    for t in trades:
        e = t.get("brent_wti_spread","unknown"); extra_dist[e] = extra_dist.get(e,0)+1
    return {
...
def print_report():
    ...
    print("\n" + "="*58)
    print(f"  BRENT_N6 Robot — Performance Report")
    print("="*58)
    ...
    print(f"  Spread dist    : {stats.get('extra_dist',{})}")
...
def main():
    parser = argparse.ArgumentParser(description="BRENT_N6 Execution Tracker")
    parser.add_argument("--report", action="store_true")
    parser.add_argument("--sync",   action="store_true")
    args = parser.parse_args()
    if args.sync:
        fills = sync_fills()
        print(f"Open BRENT_N6 positions (magic={OUR_MAGIC}): {len(fills)}")
