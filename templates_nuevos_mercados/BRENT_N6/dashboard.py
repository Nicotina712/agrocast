"""BRENT_N6 — Web Dashboard (port 8087)"""
...
def _build_html():
    ...
    for t in reversed(trades[-20:]):
        ...
        reg = t.get("oil_regime","?"); ext = t.get("brent_wti_spread","?")
        rows += f"""<tr>
          <td>{ts}</td>
...
<title>{EMOJI} BRENT_N6 Dashboard</title>
<style>
...
<div class="sig-card">
  ...
  | Régimen: {sig.get("oil_regime","—")} | Contexto: {sig.get("brent_wti_spread","—")}</small><br>
...
def start_dashboard(port=PORT):
    ...
    print(f"BRENT_N6 dashboard: http://localhost:{port}")
    srv.serve_forever()
