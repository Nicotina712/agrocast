...
# Forzar single-seed y desactivar Granger durante backtest (queremos comparación pura)
...
def main():
    ...
    out_path = os.path.join(ROOT, "artifacts_eval", "backtest_multiregime.json")
    ...
    rows_all.to_csv(os.path.join(ROOT, "artifacts_eval", "backtest_multiregime.csv"), index=False)
    print(f"💾 backtest_multiregime.csv ({len(rows_all)} filas)")
